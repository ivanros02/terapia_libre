...
class Turno {
    ...
    static async notificarTurnoConfirmado(id_turno) {
        ...
        const asunto = `✅ Turno confirmado para el ${fechaLocal} a las ${horaLocal}`;

        const mensaje = `
            Hola,
    
            Este mensaje es para confirmar el turno reservado para el día ${fechaLocal} a las ${horaLocal}.
    
            - Profesional: ${turno.nombre_profesional}
            - Paciente: ${turno.nombre_paciente}
    
            Gracias por usar TerapiaLibre.
        `;

        // 🔹 Mail al paciente
        await transporter.sendMail({
            from: '"TerapiaLibre" <terapialibre@terapialibre.com.ar>',
